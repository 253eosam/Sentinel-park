...
const ListTable: React.FC<IProps> = ({ columns, rows, watchChecked }) => {
  ...
  const isCheckedAll = useMemo(
    () => checkedArr.length === rows.length,
    [checkedArr.length, rows.length]
  )
  const notifyCheckedWatcher = (
    id: number,
  ...
  const TableHeaderCell = (column: IColumn): any => (
    <TableCell variant="head" align="center" sx={{ ...column.style }}>
      {column.type === "checkbox" ? (
        <Checkbox
          onChange={(event) =>
            notifyCheckedWatcher(CHECKBOX_ALL_EVENT_VALUE, event)
          }
  ...
  const TableBodyCell = (row: IRow) => {
    const mappingHeaders = columns.map(({ field, type, style }) => ({
      val: row[field],
      field,
      type,
      style,
    ...
    const typeCells = {
      ...
      checkbox: (val: boolean) => (
        <Checkbox
          onChange={(event) => notifyCheckedWatcher(row.id, event)}
          checked={checkedArr.includes(row.id)}
        />
      ),
    }
    return (
      <TableRow>
        {mappingHeaders.map((item) => (
          <TableCell
            ...
            sx={item.style}
          >
            {item.type && typeCells[item.type]
              ? (typeCells[item.type] as (param: any) => string)(item.val)
              : typeCells.string(item.val)}
          </TableCell>
        ))}
      </TableRow>
    )
